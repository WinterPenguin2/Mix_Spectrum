...
def main(args):
        ...
        for step in range(start_step, args.train_steps+1):
                ...
                # Run training update
                if step >= args.init_steps:
                        obs_,action_,reward_,next_obs_,not_done_=replay_buffer.sample()
                        srm_out=augmentations.random_mask_freq_v1(obs_,args)
                        image=obs_[0,:3,:,:].detach().cpu()
                        ...
                        image=srm_out[0,:3,:,:].detach().cpu()
                        image=image.permute(1,2,0).numpy()
                        import matplotlib.pyplot as plt
                        plt.imshow(image/255)
                        plt.show()

                        srm_out=augmentations.mix_freq(obs_,args)
                        image=srm_out[0,:3,:,:].detach().cpu()
                        image=image.permute(1,2,0).numpy()
                        plt.imshow(image/255)
                        plt.show()
